...
import {
  postRequestStatusItemAdsThunkCreator
} from "@/redux/reducers/item-reducer.ts";
import {
  Modal,
  ModalBody,
  ModalCloseButton,
...
const ReqestModal: React.FC<{ id: string, isOpen: boolean, onClose: () => void }> = ({ id, isOpen, onClose }) => {
  ...
  const handleClick = async () => {
    await dispatch(postRequestStatusItemAdsThunkCreator(id, valueReject));
    onClose();
  };
        ...
        <ModalContent justifyContent="center" alignItems="center" mt="300px">
          <ModalBody>
            ...
            <Fieldset.Root size="lg" maxW="md" bg="gray.50" borderRadius="10px" border="1px solid gray" p="5px">
              ...
              <Stack>
                <Fieldset.Legend>Вернуть на доработку</Fieldset.Legend>
              </Stack>
